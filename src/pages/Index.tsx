...
const Index = () => {
  const { laws, isLoading, error } = useApp();
  ...
  const getCurrentArticle = () => {
    if (searchMode === "search" && searchResults) {
      return searchResults.article
        ? { lawId: searchResults.lawId, articleNumber: searchResults.article.number }
        : null;
    }
  ...
  const navigateArticle = (direction: "prev" | "next") => {
    ...
    const newIndex =
      direction === "prev"
        ? Math.max(0, browseSelection.articleIndex - 1)
        : Math.min(law.articles.length - 1, browseSelection.articleIndex + 1);
          ...
          <div className="space-y-6">
            <Tabs value={searchMode} onValueChange={(v) => setSearchMode(v as "search" | "browse")}>
              ...
              <TabsContent value="browse" className="pt-4">
                <BrowseView
                  laws={laws}
                  selectedLaw={browseSelection?.lawId}
                  selectedIndex={browseSelection?.articleIndex}
            ...
            {!currentArticle && !isLoading && (
              <div className="flex flex-col items-center justify-center py-16 text-center">
                ...
                <h2 className="text-2xl font-serif mb-2">WADMECON2025 PRO</h2>
    ...
    <div className="space-y-4">
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div>
          ...
          <select
            id="law-select"
            className="w-full rounded-md border-border bg-secondary p-2"
            value={currentLawId}
            onChange={(e) => {
        ...
        <div className="flex items-center space-x-2">
          ...
          <Button
            ...
            disabled={
              !selectedIndex ||
              !currentLaw ||
              selectedIndex >= currentLaw.articles.length - 1
            }
          >
            Próximo Artigo
          </Button>
      ...
      {currentLaw && (
        <div className="mt-4">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-2">
            {currentLaw.articles.map((article: any, index: number) => (
              <Button
                key={article.id}
                variant={selectedLaw === currentLawId && selectedIndex === index ? "default" : "outline"}
                className="h-auto py-2 justify-start"
                onClick={() => onSelect(currentLawId, index)}
              >
                Art. {article.number}
              </Button>
            ))}
