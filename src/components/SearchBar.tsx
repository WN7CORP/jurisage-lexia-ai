      ...
      <div className="flex flex-col space-y-4 p-4 shadow-neo bg-gray-950 rounded-lg">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <div className="md:col-span-2">
            ...
            <Select value={selectedLawId} onValueChange={value => {
            ...
            setError(null);
              ...
              <SelectContent>
                {laws.map(law => <SelectItem key={law.id} value={law.id}>
                    {law.title}
                  </SelectItem>)}
              </SelectContent>
            </Select>
          ...
          <div>
            ...
            <div className="relative">
              <Input id="article-input" type="text" value={articleNumber} onChange={e => {
              setArticleNumber(e.target.value);
              setError(null);
            }} onKeyPress={handleKeyPress} placeholder="Nº do artigo" className="bg-secondary pr-8" />
              {articleNumber && <button type="button" onClick={() => setArticleNumber("")} className="absolute right-2 top-1/2 transform -translate-y-1/2 text-muted-foreground hover:text-foreground">
                  <X className="h-4 w-4" />
